...
fn transpile_all(py_src: &str, formulas: &HashMap<String, OperatorFormula>) {
    ...
    generated.push_str("#![allow(unused_variables, unused_mut, unused_assignments, unused_parens, unreachable_code, non_snake_case, clippy::excessive_precision, clippy::unnecessary_cast, clippy::needless_return, clippy::collapsible_if, clippy::collapsible_else_if, clippy::double_parens, clippy::if_same_then_else, clippy::nonminimal_bool, clippy::overly_complex_bool_expr, clippy::neg_multiply, clippy::assign_op_pattern, clippy::eq_op, clippy::get_first)]\n\n");
    ...
    for (char_id, class_name, _formula) in &custom_ops {
        ...
        // Transpile the full skill_dps body to Rust
        let rust_fn_name = to_snake_case(class_name);
        let rust_body = transpile_skill_dps(&skill_dps_body);
...
fn transpile_skill_dps(py_body: &str) -> String {
...
fn transpile_expressions(line: &str, declared: &mut std::collections::HashSet<String>) -> String {
    ...
    // int(x) → truncate toward zero (stays f64)
    let int_re = Regex::new(r"\bint\(([^)]+)\)").unwrap();
    s = int_re.replace_all(&s, "($1).trunc()").to_string();
